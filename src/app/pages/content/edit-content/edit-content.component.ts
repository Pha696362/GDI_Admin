...
import { checkExistSlug } from 'src/app/services/fire-validators.service';
import { ICreateBy } from 'src/app/interfaces/user';
import { pushToArray } from 'src/app/services/utils.lib';
...
export class EditContentComponent implements OnInit {
  ...
  buildForm(): void {
    this.form = this.fb.group({
      name: [this.data.name],
      slug:[this.data.slug.split('-').join(' ')],
  ...
  async ngOnInit() {
    ...
    this.subCategory = this.data.sub_category? this.data.sub_category.name : null;
    this.store.fetchContentSubCategory(this.data.category.key);
    // console.log(this.type_lists);
    // this.status.value.patchValue(this.data.status);
  }
  ...
  create(f: any, isNew) {
    if (this.form.valid) {
      // let data;
      // this.afs.collection("content", ref => ref.where("name", "==", f.name));
      // console.log(data,'data from filter')
      // this.store.CheckExistContent(f.name,f.slug);
