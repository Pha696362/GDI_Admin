...
import { PAYMENT_TYPES, Advertise_Status, WEB_ADVERTISEMENT_TYPE } from 'src/app/dummy/status';
...
@Component({
  selector: 'app-edit-web-advertisement',
  templateUrl: './edit-web-advertisement.component.html',
  styleUrls: ['./edit-web-advertisement.component.scss']
})
export class EditWebAdvertisementComponent implements OnInit {
  ...
  constructor(
    public dialogRef: MatDialogRef<EditWebAdvertisementComponent>,
  ...
  buildForm(): void {
    this.form = this.fb.group({
      ...
      advertiseType: [WEB_ADVERTISEMENT_TYPE.find(ads => ads.key === this.data.advertiseType.key)],
  ...
  create(f: any, isNew) {
    if (this.form.valid) {
      ...
      this.store.update(this.ds.webAdvertiseRef(), item, (success, error) => {
        if (success) {
          if (!isNew) this.dialogRef.close();
          this.snackBar.open("Web Advertisement has been created.", "done", {
            duration: 2500
          });
          this.form.enable();
          this.form.reset();
